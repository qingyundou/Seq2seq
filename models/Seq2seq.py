...
from .DecRNN import DecRNN, DecRNN_pass2
...
class Seq2seq_pass2(Seq2seq):
	def __init__(self,
		...
		self.encoder_output_p1 = EncRNN(
			...
			batch_first=batch_first,
			max_seq_len=max_seq_len,
			load_embedding_src=load_embedding_tgt,
			src_word2id=tgt_word2id,
			src_id2word=tgt_id2word
		)

		self.decoder = DecRNN_pass2(
			vocab_size_dec,
			embedding_size_dec=embedding_size_dec,
			embedding_dropout=embedding_dropout,
			hidden_size_enc=hidden_size_enc,
	...
	def forward(self, src, src_lens=None, tgt=None,
		hidden=None, is_training=False, teacher_forcing_ratio=1.0,
		beam_width=1, use_gpu=True, output_p1=None, output_p1_lens=None):
		...
		# import pdb; pdb.set_trace()
		enc_outputs = self.encoder(src, src_lens=src_lens, use_gpu=use_gpu)
		enc_outputs_o1 = self.encoder_output_p1(output_p1, src_lens=output_p1_lens, use_gpu=use_gpu)
		# pdb.set_trace()		
		...
		decoder_outputs, dec_hidden, ret_dict = self.decoder(
			...
			teacher_forcing_ratio=teacher_forcing_ratio,
			beam_width=beam_width, use_gpu=use_gpu,
			enc_outputs_o1=enc_outputs_o1, output_p1=output_p1
		)
