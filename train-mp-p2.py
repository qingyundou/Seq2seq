...
from models.Seq2seq import Seq2seq, Seq2seq_pass2
from trainer.trainer import Trainer, Trainer_pass2
...
def load_arguments(parser):
	...
	parser.add_argument('--load_p2', type=str, default=None, help='model_pass2 load dir')
	parser.add_argument('--init_dir', type=str, default=None, help='pretrained model dir')
	parser.add_argument('--init_dir_p2', type=str, default=None, help='pretrained model_pass2 dir')
...
def main():
	...
	if config['load']:
		resume = True
		print('resuming 1st pass {} ...'.format(config['load']))
		print('resuming 2nd pass {} ...'.format(config['load_p2']))
	...
	# contruct trainer
	t = Trainer_pass2(expt_dir=config['save'],
					...
					init_dir_p2=config['init_dir_p2'])
	...
	seq2seq_p2 = Seq2seq_pass2(vocab_size_enc, vocab_size_dec,
					share_embedder=config['share_embedder'],
					embedding_size_enc=config['embedding_size_enc'],
					embedding_size_dec=config['embedding_size_dec'],
					embedding_dropout=config['embedding_dropout'],
	...
	seq2seq_p2 = seq2seq_p2.to(device=device)

	# run training
	seq2seq, seq2seq_p2 = t.train(train_set, seq2seq, seq2seq_p2, num_epochs=config['num_epochs'], resume=resume, dev_set=dev_set)
